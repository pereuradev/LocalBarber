...
executarApi(static function () use ($pdo): array {
    ...
    if ($metodo === 'PATCH' && array_key_exists('status', $dados)) {
    ...
    $servico = obterEntidadeDaBarbearia(
        $pdo,
        'servicos',
        $identificadorServico,
        $identificadorBarbearia
    );
    obterEntidadeDaBarbearia(
        $pdo,
        'funcionarios',
    ...
    $identificadorCliente = trim((string)($dados['cliente_id'] ?? ''));
    ...
    if ($metodo === 'POST') {
        $gravacao = $pdo->prepare(
            'insert into agendamentos (
                ...
                :barbearia_id,
                concat(\'AG-\', upper(substr(replace(gen_random_uuid()::text, \'-\', \'\'), 1, 8))),
                :cliente_id, :servico_id, :funcionario_id,
                :cliente, :telefone, :servico,
                :data_agendamento, :horario_inicio,
                (cast(:horario_inicio_fim as time) + make_interval(mins => :duracao_fim))::time,
                \'pendente\', :observacoes, :valor_previsto
             )
             returning id, codigo'
        );
        $parametros = [];
        ...
        $gravacao = $pdo->prepare(
            'update agendamentos
             set cliente_id = :cliente_id,
                 ...
                 telefone_cliente_snapshot = :telefone,
                 servico_snapshot = :servico,
                 data_agendamento = :data_agendamento,
                 horario_inicio = :horario_inicio,
                 horario_fim = (cast(:horario_inicio_fim as time) + make_interval(mins => :duracao_fim))::time,
    ...
    $gravacao->execute(array_merge($parametros, [
        ...
        'horario_inicio_fim' => $horario,
        'duracao_fim' => (int)$servico['duracao_minutos'],
        'observacoes' => $observacoes,
        'valor_previsto' => $servico['preco'],
    ]));
