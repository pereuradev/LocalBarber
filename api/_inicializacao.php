...
final class ExcecaoApi extends RuntimeException
{
    public function __construct(
        string $mensagem,
        public readonly int $statusHttp = 400,
        public readonly string $codigo = 'requisicao_invalida'
    ) {
        parent::__construct($mensagem);
    }
}

function responderJson(array $dados, int $statusHttp = 200): never
{
...
function executarApi(callable $operacao): never
{
    try {
        $resultado = $operacao();
...
function exigirAutenticacao(PDO $pdo): array
{
    ...
    $validadoEm = (int)($_SESSION['usuario_validado_em'] ?? 0);
    $validacaoRecente = $validadoEm > 0 && (time() - $validadoEm) < 15;

    if ($identificadorUsuario === '' || !identificadorUuidValido($identificadorBarbearia)) {
    ...
    if (identificadorUuidValido($identificadorUsuario) && !$validacaoRecente) {
        $consultaUsuario = $pdo->prepare(
            'select
                u.id, u.barbearia_id, u.nome, u.email, u.papel,
                b.nome_fantasia, b.cor_tema
             from usuarios u
             left join barbearias b on b.id = u.barbearia_id
             where u.id = :usuario_id
               and u.barbearia_id = :barbearia_id
               and u.ativo
             limit 1'
        );
        ...
        if (!$usuario) {
            invalidarSessao();
            throw new ExcecaoApi(
                'Seu acesso foi desativado. Entre novamente ou fale com um administrador.',
                401,
                'acesso_desativado'
            );
        }
        ...
        !identificadorUuidValido($identificadorUsuario)
        && (
            !str_starts_with($identificadorUsuario, 'legacy-')
            || ($_SESSION['usuario_papel'] ?? null) !== 'admin'
        )
    ) {
        invalidarSessao();
        throw new ExcecaoApi(
    ...
    $papelUsuario = (string)($_SESSION['usuario_papel'] ?? 'admin');
    tokenCsrf();

    $sessao = [
        'usuario_id' => $identificadorUsuario,
        'barbearia_id' => $identificadorBarbearia,
        'usuario_nome' => (string)($_SESSION['usuario_nome'] ?? 'Usuário'),
        'usuario_email' => (string)($_SESSION['usuario_email'] ?? ''),
        'usuario_papel' => $papelUsuario,
        'tipo_acesso' => tipoAcessoPorPapel($papelUsuario),
        'permissoes' => permissoesPorPapel($papelUsuario),
        'barbearia_nome' => (string)($_SESSION['barbearia_nome'] ?? 'LocalBarber'),
        'barbearia_cor_tema' => (string)($_SESSION['barbearia_cor_tema'] ?? '#244BC5'),
    ];
